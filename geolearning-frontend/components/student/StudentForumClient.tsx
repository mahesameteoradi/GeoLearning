...
import { MessageSquare, Pin, Clock, Reply, ChevronDown, ChevronUp, Send, Loader2, Trash2, Globe, BookOpen, User } from 'lucide-react'
  ...
  const handleReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('forum_replies')
        .insert({ post_id: post.id, user_id: userId, body: replyText.trim() })
...
export function StudentForumClient({
  posts: initialPosts,
  classes,
  userId,
}: {
    ...
    <div>
        ...
        <div className="flex flex-1 gap-1.5 overflow-x-auto">
          {[
            ...
            <button
              ...
              className={cn(
                'whitespace-nowrap rounded-xl border px-3 py-1.5 text-xs font-semibold transition-all',
                filterClassId === opt.id
      ...
      {filteredPosts.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 py-20 text-center">
          ...
          <p className="text-sm font-medium text-slate-500">Belum ada diskusi</p>
          <p className="mt-1 text-xs text-slate-600">Pilih tab lain atau tunggu guru memposting</p>
        </div>
      ) : (
        <div className="space-y-3">
