    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/80 backdrop-blur-lg border-b border-slate-200/50 shadow-sm py-3' : 'bg-transparent py-5'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between transition-all">
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="text-2xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-700 tracking-tight">GeoLearning</span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8">
          <div className="flex items-center gap-6">
            {links.map((link) => (
              <Link key={link.name} href={link.href} className="text-sm font-semibold text-slate-600 hover:text-indigo-600 transition-colors relative group">
                {link.name}
                <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-indigo-600 transition-all duration-300 group-hover:w-full rounded-full"></span>
              </Link>
            ))}
          </div>
          <Link 
            ...
            className="relative overflow-hidden bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm transition-all shadow-md shadow-indigo-500/20 hover:-translate-y-0.5"
          >
        ...
        {/* Mobile Menu Button */}
        <button 
          className="md:hidden p-2 text-slate-600 hover:text-indigo-600 bg-white/50 backdrop-blur-sm rounded-lg"
              ...
              <div className="flex flex-col gap-4">
                {links.map((link) => (
                  <Link 
                    ...
                    className="text-lg font-semibold text-slate-600 hover:text-indigo-600"
                  >
                    {link.name}
                  </Link>
                ))}
                <div className="pt-4 border-t border-slate-100">
                  <Link 
                    ...
                    className="flex items-center justify-center w-full bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-3.5 rounded-xl font-bold transition-all shadow-md"
                  >
                    {userRole ? 'Ke Dashboard' : 'Masuk'}
                  </Link>
                </div>
