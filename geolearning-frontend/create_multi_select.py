...
with codecs.open('d:/SEMESTER 8/geo_LearningMedia/geolearning-frontend/components/teacher/ResourceBankModal.tsx', 'w', encoding='utf-8') as f:
    f.write(''''use client'
...
export function ResourceBankModal({ classId, targetModuleId: initialModuleId, existingModules, nextOrderMap = {}, onClose, onSuccess }: ResourceBankModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    const isConfirmed = await confirm({
      ...
      message: \Apakah Anda yakin ingin menambahkan \ materi yang dipilih ke kelas ini?\,
    ...
    try {
      ...
      toast.success(\Berhasil menambahkan \ materi!\, { id: tid })
      onSuccess()
    } catch (err: any) {
