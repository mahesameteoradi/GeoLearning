...
export class AdminService {
  ...
  async createTeacher(data: any) {
    ...
    if (!authData.user)
      throw new BadRequestException('Gagal membuat user auth');
  ...
  async updateTeacher(teacherId: string, data: any) {
    ...
    if (data.name)
      authUpdates.user_metadata = { full_name: data.name, role: 'TEACHER' };
    ...
    if (authError) {
      throw new BadRequestException(
        'Gagal memperbarui data otentikasi: ' + authError.message,
      );
    }
